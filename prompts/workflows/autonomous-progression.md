...
Before substantive `/go` lifecycle progression, resolve the ephemeral `/go` Resolved Agent Run Context from current authoritative inputs using [Resolved agent run context](resolved-agent-run-context.md). Bind the governing objective, current lifecycle/candidate/result identity, current review/evidence state where applicable, authority and instruction provenance, effective/prohibited capabilities, owner-decision boundaries, continuation mode, proposed next governed action, required preconditions/evidence, and completion conditions. Treat `next_governed_action` as a proposal until the exact action passes the `/go` action gateway.

Before every consequential `/go` transition, refresh the decision-critical state capable of invalidating the action, re-resolve as needed, and classify the exact proposal as `ALLOW`, `REQUIRE OWNER / SEPARATE AUTHORITY`, or `FORBID`. Execute only an `ALLOW` action that is actually available. Missing or ambiguous authority never defaults to `ALLOW`; technical capability availability never creates authority. If an `ALLOW` action cannot execute in the current environment, preserve the capability / `EXTERNAL_REQUIRED` boundary rather than manufacturing a new owner-authority decision.

When an `ALLOW` action uses a capability with a Promptbook availability key, consume the one `resolved_capability_availability` record derived by the resolved-run-context / [Capability availability overrides](capability-availability-overrides.md) chain after authority classification and before invocation or projection. Do not independently rediscover Project, repository, work-item, or prior-conversation availability declarations inside autonomous progression. A capability configured as unavailable, or conservatively unresolved because of invalid, contradictory, stale, target-mismatched, or required-but-unavailable configuration, must not be invoked or probed. An `enabled` value removes only Promptbook suppression; it does not create authority, prove executor support, or prove success. Preserve the resolved configuration provenance when it changes execution or fallback behaviour, and verify the resulting state after any attempted capability transition.

For the initial `pull_request.mark_ready` key, avoid creating a transition merely out of habit. If implementation is complete, required validation has passed, no genuine lifecycle hold remains, and independent review is the next governed gate, a pull request may be created as non-draft directly. If work, validation, an unresolved decision, repository policy, or another real hold remains, preserve legitimate draft state. When such a draft later becomes reviewable and `pull_request.mark_ready` is configured as unavailable or fails at runtime despite being enabled, do not repeatedly invoke it; use the bounded `EXTERNAL_REQUIRED` ready-for-review hand-off defined by the availability contract. Never use non-draft creation to bypass a real hold.
...
4. executable with capabilities legitimately available in the current environment after any applicable resolved capability-availability filter.
