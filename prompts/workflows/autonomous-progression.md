...
When an `ALLOW` action uses a capability with a Promptbook availability key, apply [Capability availability overrides](capability-availability-overrides.md) after authority classification and before invocation or projection. A capability configured as unavailable must not be invoked or probed. Unknown, contradictory, stale, or otherwise unresolved effective availability fails conservatively for that capability. An `enabled` value removes only Promptbook suppression; it does not create authority, prove executor support, or prove success. Preserve the effective configuration provenance when it changes execution or fallback behaviour, and verify the resulting state after any attempted capability transition.
...
4. executable with capabilities legitimately available in the current environment after any applicable capability-availability override.
