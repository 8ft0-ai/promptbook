...
- `PROJECT_AMBIGUOUS` — strong target evidence conflicts, suggests another project without a safely identifiable target, or the configured project identity is missing/unresolved. Perform no substantive work or tool call. State the ambiguity and require the same explicit acknowledgement before proceeding here intentionally. A prompt that merely omits a project name is not ambiguous when it naturally continues this project's established work.
